...
//Výplata Dále vytvořte funkci taxed, která na vstupu obdrží částku a procento zdanění, a vrátí částku v celých korunách zdaněnou podle zadaných procent.

const salary = (wage, hours, days) => {
	return Math.round(wage * hours * days)
}

const taxed = (taxBase, taxRate) => {
	return Math.round((taxRate)-(taxBase * taxRate))
}
...
console.log(taxed(taxRate, taxBase))

//Kalkulačka Napište funkci calculate se třemi parametry number1, operation a number2, které představují první zadané číslo, zadanou operaci jako řetězec a druhé zadané číslo. Operace může být '+', '-', '*' nebo '/'. Funkce vrátí výsledek výpočtu pro zadanou operaci.
